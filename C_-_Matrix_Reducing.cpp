...
void solve(){
  ...
  for(int j=0;j<(1<<w1);j++){
        ...
        vi rows,col;
         for(int k = 1; k <= h1; k++) if((i & (1<<(k-1))) == 0) rows.push_back(k);
        for(int k = 1; k <= w1; k++) if((j & (1<<(k-1))) == 0) col.push_back(k);
        if(rows.size() != h2 || col.size() != w2) continue;

         bool match = true;
      for(int k = 1; k <= h2; k++){
        for(int l = 1; l <= w2; l++){
          if(a[rows[k-1]][col[l-1]] != b[k][l]){
            match = false;
            break;
          }
        }
